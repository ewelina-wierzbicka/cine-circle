...
- **Grid posters preload the first `PRIORITY_CARD_COUNT` cards only** (`MediaList`, currently 6). A preload link fires at every viewport width, but the grid runs 2 columns on mobile and 6 at `xl`. Six is one full row at `xl` and roughly the mobile fold. Do not raise it: at `grid-cols-2` a higher count preloads rows the phone never paints. Cards past the cutoff stay lazy, which is the only viewport-aware loading the browser offers.
