...
| File                 | Tests                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auth.spec.ts`       | T1 login + logout, T1b logout signs out current device only, T2 login validation, T3 registration validation + confirm-email, T4 confirm-callback → registration-confirmed, T5 confirm-callback no code → login toast, T6 registration-confirmed redirects logged-in user, T7 auth redirect + rurl, T8 forgot-password, T9 reset-callback failure, T10 reset-password validation, T11 reset-password rejects non-recovery session, T12 reset happy path |
...
- Any test that logs out must use `isolatedUser`, never the shared persistent user. `logout()` now signs out with `scope: 'local'`, but a shared user still gets its session churned by a parallel test; keep logout tests isolated.
...
- `webServer.command` derives its port from `E2E_BASE_URL` (`npm run dev -- -p <port>`). Without it `next dev` binds 3000 while the suite waits on 3001, and every run dies at the 120s webServer timeout.
