...
- `e2e/global-setup.ts` warms the routes the suite visits before any test runs. A cold `next dev` compile of `/movie/[id]` alone takes ~30s, which used to blow the per-test timeout. Add a route there when a new spec navigates somewhere new: `WARM_ROUTES` for open and auth-only routes, `WARM_ROUTES_AUTHED` for anything behind the proxy auth check (an anonymous warm-up is redirected to `/login` and never compiles the page).
