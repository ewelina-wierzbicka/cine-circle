...
type Props = { item: NormalizedMedia; idx?: number };

export default function SearchDropdownItem({ item }: Props) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-3 w-full px-4 py-2 hover:bg-bg3 text-left text-primary text-sm border-b border-white/5 last:border-0 cursor-pointer"
    >
      {item.poster_path ? (
        <Image
