...
- Implemented by `src/app/(auth)/error.tsx` (client component, auth error boundary), `src/app/(app)/error.tsx` (client component), `src/app/(app)/not-found.tsx` (client component), and `src/app/not-found.tsx` (client component, root 404).
...
- `(auth)/error.tsx` renders inside the bare `(auth)/layout.tsx` (`h-screen`), so it carries its own full-screen backdrop replicating the `AuthFormLayout` left column: `bg-dark` wrapper, the 48px grid pattern overlay at `opacity-40`, and a mint radial glow pinned to the bottom center. Content sits in a `max-w-90 animate-fade-up` column echoing the auth form pages: a mint-tinted `AlertCircleIcon` in a `rounded-xl border border-mint/20 bg-mint/10` tile, a `font-serif text-4xl` title with mint italicized word (`Something went / _wrong_`), `text-sm text-secondary` body copy, a full-width mint `Button` ("Try again" → `reset()`), and a `Back to Sign in` link. When `error.digest` is present it shows as `font-mono text-xs tracking-[0.08em] text-secondary/50`.
