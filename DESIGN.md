...
- Info view (`MediaInfo` or `WatchedMediaInfo`) shows:
  ...
  - Genre pills (when available): flex-wrap row of `font-mono text-xs tracking-[0.08em] uppercase px-2.5 py-1 rounded-full border border-white/10 text-mint` spans. Placed above the title, below the type label.
  ...
  - Overview text (when available): `text-sm text-secondary leading-relaxed mb-8`, placed below the mint divider and above the action buttons (or above the rating section in `WatchedMediaInfo`).
...
- Genre badge overlay: `font-mono text-xs` in a `bg-dark/70 text-mint` pill at bottom-left of the card.
- Title below card: `text-xs text-secondary truncate`, transitions to `text-primary` on hover.
